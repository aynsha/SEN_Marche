...
module.exports.signUp = async (req, res) => {
    try {
        const { firstName, lastName, numberPhone, email, password, role } = req.body;
...
// Commentaires
// Ce code contient deux fonctions, `signUP` et `signIn`, qui permettent de gérer les opérations d'inscription et de connexion d'un utilisateur dans une application utilisant Node.js et MongoDB (via le modèle `User`). Je vais commenter chaque partie de ce code :
...
//     - Elle commence par récupérer les données de l'utilisateur (prénom, nom, numéro de téléphone, email, mot de passe, rôle) depuis `req.body`.
//     - Elle vérifie si un utilisateur avec cet email existe déjà dans la base de données en utilisant `User.findOne`.
